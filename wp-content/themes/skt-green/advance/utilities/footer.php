...
if(!function_exists('posterity_theme_footer')){
	function posterity_theme_footer(){
		...
		if ( ! function_exists( 'elementor_theme_do_location' ) || ! elementor_theme_do_location( 'footer' ) ) {
			...
			$footer_logo = $posterity_a13->get_option( 'footer_logo' );
			$footer_logo_link = $posterity_a13->get_option( 'footer_logo_link' );
			...
			if(strlen($output)){
				...
				$html = '<footer id="footer" class="'.esc_attr($footer_class).'"'.posterity_get_schema_args('footer').'>';
				if(!empty($footer_logo)){
					...
					$html .= posterity_social_icons($posterity_a13->get_option( 'footer_socials_color' ), $posterity_a13->get_option( 'footer_socials_color_hover' ));
					$html .= '</div>';					
					}
		...
		global $posterity_a13; ?>
			<div class="foot-items">
				<div class="foot-content clearfix">
					...
					?>
					<div class="credtext">
                    	<?php esc_html_e('By ','skt-green');?><a class="credlink" href="<?php echo esc_url('https://www.sktthemes.org/');?>" rel="nofollow" target="_blank"><?php esc_html_e('SKT Themes','skt-green'); ?></a>
                    </div>
					<?php
